...
export default function NewsCard({ data, isTopNews }: NewsCardProps) {
    return (
        <div className={`bg-white rounded-2xl p-6 border ${isTopNews ? 'border-brand-orange ring-1 ring-brand-orange shadow-[0_0_20px_rgba(255,106,61,0.15)]' : 'border-gray-100 shadow-soft hover:shadow-card'} hover:-translate-y-1 transition-all duration-300 flex flex-col h-full group relative`}>
            ...
            {/* Title & Summary */}
            <h3 className="font-hero font-bold text-xl text-brand-dark mb-3 leading-tight group-hover:text-brand-orange transition-colors">
                {data.title}
            </h3>
            ...
            <div className="mt-auto grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6 bg-brand-bg p-4 rounded-xl">
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500 font-noname line-clamp-2" title={data.ssb_importance}>{data.ssb_importance}</p>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500 font-noname line-clamp-2" title={data.gd_topic}>{data.gd_topic}</p>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500 font-noname line-clamp-2" title={data.lecturette}>{data.lecturette}</p>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500 font-noname line-clamp-2" title={data.interview_question}>{data.interview_question}</p>
                </div>
            </div>
