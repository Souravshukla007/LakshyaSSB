...
async function fetchSingleQuery(apiKey: string, query: string): Promise<any[]> {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`[gnews-fetcher] API error ${response.status} for query "${query}":`, errorText);
            return [];
        }

        const data = await response.json();
        const articles: any[] = data.articles || [];
        console.log(`[gnews-fetcher] Query "${query}" → ${articles.length} articles`);
        return articles;

    } catch (error) {
        console.error(`[gnews-fetcher] Fetch failed for query "${query}":`, error);
        return [];
    }
}
...
export async function fetchGNews(): Promise<RawArticle[]> {
    ...
    if (!apiKey) {
        console.error('[gnews-fetcher] GNEWS_API_KEY is not set');
        return [];
    }
    ...
    console.log(`[gnews-fetcher] Total unique articles after merge: ${allArticles.length}`);
    ...
    console.log(`[gnews-fetcher] ${relevant.length} articles passed keyword filter`);
